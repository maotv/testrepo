...
<li class="STYLE CSERVICE">LINK\
	...
	proto.startEmpty = function() {	
		...
		this.v.pollStreamInitialData(this.stream, function(envelopeArray, metaData) { 
			...
			// if we have a satellite component (target)
			if (self.target) {
				...
				self.container.closest('#fxContainer').css({visibility: 'visible'});
				if (self.data.length < 1  && self.container) {
					self.container.closest('#fxContainer').hide();
				} else if(self.data.length < 2 && self.container) {
					self.container.hide();
					self.container.closest('#fxContainer').show();
