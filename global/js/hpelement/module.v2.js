var Flyp = (function(fl, $_){
	...
	function Module(env) {
		...
		if (this.w == 0) {
			if (document.flypinit) document.flypinit++;
			else document.flypinit = 1;
			
			if (document.flypinit > 2) return;
		...
		this.force169  = (env.force169 === true);
...
<li class="STYLE CSERVICE">LINK\
	...
	proto.startEmpty = function() {	
		...
		this.v.pollStreamInitialData(this.stream, function(envelopeArray, metaData) { 
			...
			for(var i in self.data) {
				...
				if (m && m.link && String(m.link).indexOf("externalPlayer") > -1) {
	...
	proto.buildElement = function(envelope) {
		...
		elem = this.template
			...
			.replace(/USERICON/, m.user && m.user.icon?'<img onerror="this.style.display=\'none\'" class="flpicon" src="' + m.user.icon.replace('http:', '') + '"></img>':'')
	...
	proto.resize = function() {
		...
		var w = this.flypparent.width();
		if (w == this.w) return;
		this.w = w;
		var h = this.flypparent.height();
		if (h == 0 || this.force169) {
			this.h = Math.ceil(this.w * 9 / 16);
