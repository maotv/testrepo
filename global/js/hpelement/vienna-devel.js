...
;(function() {
	...
	proto.pollStreamInitialData = function(stream, callback) {
		...
		this.pollCarrierInternal(self.url + "/s/" + stream + "/carrier.json" , function(carrier) {
			self.pollInitialInternal(self.url + "/s/" + stream + "/initial.json", carrier.commit, callback);
		})
