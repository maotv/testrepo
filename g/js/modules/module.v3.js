...
<li class="STYLE CSERVICE">LINK\
	...
	proto.postMessage = function(json) {
		// der briefkasten
		console.log("json",json,this.data);
		if(this.data 
			&& this.data.length > 0 
	...
	proto.buildVideo = function(envlp) {
		...
		if(!this.v) {
			...
			if(envlp.message.media && envlp.message.media.poster) {				
	...
	proto.buildElement = function(envelope) {
		...
		if (this.target) m.link = false;  // if we have a target, we don't want the normal link behaviour
	...
	proto.resize = function() {
		...
		//console.log(this.w, h, this.force169); // || this.force169
		if (h == 0 ) this.h = Math.ceil(this.w * 9 / 16);
		else this.h = h;
		// diese zeile war auskommentiert. warum?
