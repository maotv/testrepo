var Flyp = (function(fl){
	...
	function Module(env) {
		...
		this.v = new Flyp.Vienna({
			...
			root: "//flypsite.net/events/" + env.owner + "/" + env.event + "/rio"	
		});
		...
		this.fonts     = (env.headerfontsizes)?env.headerfontsizes: {'icon': Math.round(smaller * this.parts.header * 0.66) + "px", 'text': Math.round(smaller * this.parts.header * 0.3) + "px", 'smalltext':  Math.round(smaller * this.parts.header * 0.24) + "px"};
...
<li class="STYLE CSERVICE">LINK\
	...
	<footer>\
		...
		this.fonts.lineheight = Math.round(smaller * this.parts.header * 0.3) + "px";
		
		if (this.forceclear) this.container.empty();
				
		this.start();
		
		if (this.animate && !this.failed) {
		...
		var self = this;
		if (this.autoresize) {
			$(window).resize( function() {  // resize-Events sammeln (500ms)
	...
	proto.buildElement = function(envelope) {
		...
		this.container.find("header").css({'height': this.h * this.parts.header + "px", 'font-size': this.fonts.text, 'line-height': this.fonts.lineheight});
		this.container.find(".time").css({'font-size': this.fonts.smalltext, 'line-height': this.fonts.lineheight });
		this.container.find(".service").css({'font-size': this.fonts.smalltext, 'line-height': this.fonts.lineheight});
		this.container.find(".name").css({'line-height': this.fonts.lineheight });
		this.container.find(".handle").css({'line-height': this.fonts.lineheight});
		this.container.find("footer").css({'height': this.h * this.parts.footer + "px"});
	...
	proto.floater = function(){
		...
		// load next image
		li2 = this.container.find(".flypContainer li:last-child")
		img2 = li2.find("content span.mimage");
		if (img2.length > 0) {
	...
	proto.resize = function() {
		...
		this.container.css({height: this.h + "px"});
		var smaller = Math.min (this.h, this.w * 0.666);
		this.fonts     = (this.env.headerfontsizes)?this.env.headerfontsizes: {'icon': Math.round(smaller * this.parts.header * 0.66) + "px", 'text': Math.round(smaller * this.parts.header * 0.3) + "px", 'smalltext':  Math.round(smaller * this.parts.header * 0.24) + "px"};
		this.fonts.lineheight = Math.round(smaller * this.parts.header * 0.3) + "px";
		this.container.find(".flypContainer").empty();
		this.buildAll();
		this.animfunc();
	...
	fl.getImageElem = function(image, w, h) {
		...
		var elem = $("<span class=\"mimage\"></span>").css(cssCalc);
		elem.attr('lazy', image.url.replace('http:', ''));
	...
	fl.formatTime = function(t) {
		...
		if ( diff < 1 * 3600 * 1000 ) {
			var diffrd = Math.round(diff / 1000 / 60);
			var plural = (diffrd != 1)?"n":"";
			retval = "vor " + diffrd + " Minute" + plural;
		} else {
			function pad(x) {
				return (x < 10 ? "0" + x:x );
			}
